...
namespace Schafkopf_Lib {
    class Geier : IKontrolle {
        public bool Erlaubt( Modell m, int ID ) {
            ...
            if ( angespielt.wert.Equals(Karte.Wert.OBER) ) {
                if ( tisch[ID].wert.Equals(Karte.Wert.OBER) ) {
                    return true;
            ...
            if ( tisch[ID].farbe.Equals(angespielt.farbe)
                    && !tisch[ID].wert.Equals(Karte.Wert.OBER) ) {
        ...
        */
        private bool hatKeinenTrumpf( Modell m, int ID ) {
            ...
            for ( int i = 0; i < y.Count(); i++ ) {
                if ( y[i].wert.Equals(Karte.Wert.OBER) ) {
                    return false;
                }
            }
            if ( m.Tisch[ID].wert.Equals(Karte.Wert.OBER) ) {
                return false;
            }
        ...
        private bool keineFarbe( Karte.Farbe farbe, Modell m, int ID ) {
            ...
            for ( int i = 0; i < y.Count(); i++ ) {
                karte = y[i];
                if ( karte.farbe.Equals(farbe) && !karte.wert.Equals(Karte.Wert.OBER) ) {
                    return false;
                }
            }
        ...
        public int Sieger( Modell m, int erster ) {
            ...
            for ( int i = 0; i < 4; i++ ) {
                if ( gespielt[i].wert.Equals(Karte.Wert.OBER) ) {
                    ober = true;
                }
            }
            if ( !ober ) {
